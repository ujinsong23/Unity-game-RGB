...
public class WorldRScript : MonoBehaviour
{
   // public PlayerMove playerMove;
    ...
    int scene_index = 0; 
    //scene이 완전히 독립되어 있는 Ending으로 넘어가기 위해서 빌드 세팅 내의 인덱스 값을 이용해 이동하기 위한 초기 값 지정
    //RGB가 있는 GameScene이 0번 빌드이고 Ending씬이 1번 빌드이기 때문에 +1 연산으로 인덱스를 넘어가기 위해 초기 값을 0으로 지정함
    ...
    void Start()
    {
       ...
       // playerMove = GameObject.Find("Player").GetComponent<PlayerMove>();
        ...
        audioSource = GameObject.Find("WorldR").GetComponent<AudioSource>();
    ...
    void Update()
    {
        if (Input.GetButtonDown("Fire2")) SceneManager.LoadScene("Ending");
        ...
        if (goal)
        {
            ...
            objectR = false;
            SceneManager.LoadScene("lightGather");
    ...
    /*
    public void SceneLoad() //SceneManager 컴포넌트를 이용하여 초기 인덱스 값 0에 +를 하여 1번 인덱스(Ending)로 넘어가기 위한 메서드 정의 
    {
